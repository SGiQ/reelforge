...
export default function ScriptCard({ id, title, slides, selected, onSelect }: ScriptCardProps) {
    return (
        <button
            ...
            style={{
                background: selected ? "rgba(198,241,53,0.12)" : "rgb(var(--rgb-surface-card) / 0.8)",
                border: `1.5px solid ${selected ? "rgba(198,241,53,0.6)" : "rgb(var(--rgb-surface-border) / 0.6)"}`,
                boxShadow: selected ? "0 0 30px rgba(198,241,53,0.15)" : "none",
            }}
        >
            <div className="flex items-start justify-between mb-4">
                <div>
                    <p className="text-xs font-semibold uppercase tracking-wider mb-1" style={{ color: selected ? "var(--color-accent)" : "var(--color-text-muted)" }}>
                        Script
                    </p>
                ...
                {slides.map((slide, i) => (
                    ...
                    >
                        <span
                            ...
                            style={{
                                background: selected ? "rgba(198,241,53,0.3)" : "rgb(var(--rgb-surface-border) / 0.6)",
